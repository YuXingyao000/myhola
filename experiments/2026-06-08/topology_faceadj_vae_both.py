#!/usr/bin/env python3
"""Topology face-adj VAE with per-face degree tokens AND prefix corruption.

Combines two changes on top of the 06-06 edge-count baseline:

1. Per-face degree tokens with degree-budget constrained decoding (same as
   `topology_faceadj_vae_degree.py`). Adds a `[DEG_0..DEG_{max_faces-1}]`
   block between the global counts and the pair tokens, plus a degree CE
   loss. Generation uses both edge-count and per-face degree budgets as
   hard constraints.

2. Prefix corruption (training-only): with probability `corrupt_prob`, the
   NO_EDGE / EDGE pair tokens in the decoder input are flipped while the
   target sequence is kept clean. Special tokens (BOS, N_FACE, N_EDGE,
   degree tokens, PAD) are never corrupted — degree tokens carry budget
   information consumed by constrained decoding and must stay clean.
"""
...
def corrupt_pair_prefix(decoder_input, decoder_mask, corrupt_prob):
    ...
    Special tokens (BOS, N_FACE, N_EDGE, degree tokens, PAD) carry token ids
    other than NO_EDGE/EDGE so the `pair_pos` filter naturally protects them.
    """
    if corrupt_prob <= 0:
...
def parse_args():
    ...
    parser.add_argument("--output-dir", default="experiments/2026-06-08/outputs_both")
...
def main():
    ...
    print(
        ...
        f"degree_loss_weight={args.degree_loss_weight}; corrupt_prob={args.corrupt_prob}",
        flush=True,
    )

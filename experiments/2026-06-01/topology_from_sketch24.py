#!/usr/bin/env python3
"""Toy topology predictor: 24 OCC white/sketch images -> face adjacency matrix.
...
def has_required_condition_files(cond_dir):
    if not cond_dir.exists():
        return False
    return (cond_dir / "sketch.npz").is_file() or (cond_dir / "imgs.npz").is_file()
    ...
    def __getitem__(self, index):
        ...
        for offset in range(len(self.model_ids)):
            ...
            try:
                ...
                sketch_path = self.cond_root / model_id / "sketch.npz"
                if sketch_path.is_file():
                    with np.load(sketch_path) as data:
                        images = data["images"]
                else:
                    with np.load(self.cond_root / model_id / "imgs.npz") as data:
                        images = data["sketch_imgs"]
                ...
                return {
                    ...
                    "view_ids": torch.arange(NUM_CUBE24_VIEWS, dtype=torch.long),
...
class TopologyPredictor(nn.Module):
    def __init__(self, max_faces, dino_model, hidden_dim, face_dim):
        ...
        rotation_features = np.stack(get_cube24_rotation_matrices(), axis=0).reshape(NUM_CUBE24_VIEWS, 9)
        self.register_buffer("rotation_features", torch.from_numpy(rotation_features).float())
        self.view_embedding = nn.Embedding(NUM_CUBE24_VIEWS, self.dino_dim)
        self.rotation_projection = nn.Sequential(
            nn.Linear(9, self.dino_dim),
    ...
    def forward(self, images, view_ids=None):
        ...
        if images.dim() == 5:
            ...
            cls = cls.reshape(batch_size, num_views, -1)
            if view_ids is None:
                view_ids = torch.arange(num_views, device=images.device)[None].expand(batch_size, num_views)
            view_ids = view_ids.long()
            camera_feature = self.view_embedding(view_ids)
            rotation_feature = self.rotation_projection(self.rotation_features[view_ids])
            view_feature = cls + camera_feature + rotation_feature
            view_weight = torch.softmax(self.view_attention(view_feature), dim=1)
            image_feature = (view_feature * view_weight).sum(dim=1)
        else:
...
def evaluate(model, loader, device, desc="eval"):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=desc, dynamic_ncols=True, leave=False):
            ...
            view_ids = batch["view_ids"].to(device)
            adj = batch["face_adj"].to(device)
            valid = batch["valid"].to(device)
            valid_logits, edge_logits = model(images, view_ids)
...
def run_eval_or_predict(args):
    ...
    view_ids = batch["view_ids"][None].to(device)
    with torch.no_grad():
        valid_logits, edge_logits = model(images, view_ids)
    ...
    out_base = Path(args.output_dir) / f"{args.model_id}_sketch24_topology"
    np.savez_compressed(
...
def main():
    ...
    parser.add_argument("--output-dir", default="experiments/2026-06-01/outputs_sketch24")

#!/usr/bin/env python3
"""Toy topology predictor: one OCC gray image -> face adjacency matrix.
...
def has_required_condition_files(cond_dir, view_id):
    ...
    def __init__(self, raw_root, cond_root, model_list, max_faces, view_id, max_samples=0):
        ...
        for model_id in model_ids:
            ...
            if not raw_path.is_file():
                skipped_raw += 1
                continue
            if not has_required_condition_files(self.cond_root / model_id, self.view_id):
                skipped_condition += 1
                continue
...
def run_eval_or_predict(args):
    ...
    if args.mode == "eval":
        test_set = TopologyDataset(args.raw_root, args.cond_root, args.test_list, max_faces, args.view_id, args.max_samples)
    ...
    dataset = TopologyDataset(args.raw_root, args.cond_root, one_item_list, max_faces, args.view_id, 1)
    batch = dataset[0]
    ...
    out_base = Path(args.output_dir) / f"{args.model_id}_svr_single_v{args.view_id:02d}_topology"
    np.savez_compressed(
...
def main():
    ...
    parser.add_argument("--output-dir", default="experiments/2026-06-01/outputs_svr_single")

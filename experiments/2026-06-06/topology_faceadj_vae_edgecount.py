...
def face_count_token(num_faces: int) -> int:
    return FACE_COUNT_OFFSET + int(num_faces)
...
def edge_count_offset(max_faces: int) -> int:
    return FACE_COUNT_OFFSET + max_faces + 1


def edge_count_token(edge_count: int, max_faces: int) -> int:
    return edge_count_offset(max_faces) + int(edge_count)


def token_to_edge_count(token, max_faces: int):
    if torch.is_tensor(token):
        return token - edge_count_offset(max_faces)
    return int(token) - edge_count_offset(max_faces)


def max_edge_count(max_faces: int) -> int:
...
def num_pair_tokens(max_faces: int) -> int:
    return max_edge_count(max_faces)


def sequence_length(max_faces: int) -> int:
    return 2 + num_pair_tokens(max_faces)
    ...
    def __getitem__(self, index: int):
        ...
        for offset in range(len(self.model_ids)):
            ...
            try:
                ...
                tokens, token_mask, pair_targets, pair_mask, num_faces, edge_count = adj_to_target(
                    face_adj=face_adj,
...
class FaceAdjEdgeCountTransformerVAE(nn.Module):
    ...
    def generate(
        ...
        if greedy:
            ...
            face_cls = torch.distributions.Categorical(logits=allowed_face / max(temperature, 1e-6)).sample()
        ...
        if greedy:
            edge_count_cls = allowed_edge.argmax(dim=-1)
        else:
            edge_count_cls = torch.distributions.Categorical(logits=allowed_edge / max(temperature, 1e-6)).sample()
        edge_count_tok = edge_count_cls + edge_count_offset(self.max_faces)
        ...
        for pair_idx in range(max_pairs):
            ...
            if active.any():
                ...
                if greedy:
                    edge_cls = pair_logits.argmax(dim=-1)
                else:
                    edge_cls = torch.distributions.Categorical(
                        logits=pair_logits / max(temperature, 1e-6)
                    ).sample()
...
def evaluate(model: nn.Module, dataloader: DataLoader, device: torch.device, args, epoch: int) -> dict[str, float]:
    ...
    if args.prior_samples > 0:
        ...
        prior_adj, prior_count, prior_edge_count = target_to_adj(prior_generated, max_faces=core.max_faces, min_faces=args.min_faces)

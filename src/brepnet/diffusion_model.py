...
def clip_style_symmetric_infonce(cad_emb, img_emb, tau=0.07):
    """
    cad_emb: (B, D), already normalized
    img_emb: (B, D), already normalized
    """
...
class ZeroInitImageAdapter(nn.Module):
    def __init__(self, dim_model, dim_condition, num_heads):
        super().__init__()
...
class Diffusion_condition(nn.Module):
    def __init__(self, v_conf, ):
        ...
        self.num_diffusion_layers = int(v_conf.get("num_diffusion_layers", 24))
        self.layerwise_img_adapter = bool(v_conf.get("layerwise_img_adapter", False))
        self.layerwise_img_adapter_every = max(1, int(v_conf.get("layerwise_img_adapter_every", 2)))
        ...
        if "single_img" in v_conf["condition"] or "multi_img" in v_conf["condition"] or "sketch" in v_conf["condition"]:
            ...
            # ============================================================
            # Image-condition backbone toggle
            # ------------------------------------------------------------
            # Comment out one block and uncomment the other to switch.
            # Both options expose a frozen ViT-L/14 producing [B, 257, 1024]
            # tokens (1 CLS + 16x16 patches), so `self.img_fc` and the rest
            # of the network stay unchanged. The matching forward call lives
            # in `extract_condition`; keep these two sites in sync.
            # ============================================================

            # ---- Option A (active): Depth-Anything-V2 DINOv2 ViT-L backbone ----
            # The DPT depth head is discarded; we only keep `.pretrained`,
            # which is the DINOv2 ViT-L encoder fine-tuned for depth.
            # model_configs = {
            ...
            # _da_v2.load_state_dict(torch.load(
            #     f'thirdparty/Depth_Anything_V2/checkpoints/depth_anything_v2_{encoder}.pth',
            #     map_location='cpu'))
            # self.img_model = _da_v2.pretrained
            # del _da_v2

            # ---- Option B (inactive): vanilla DINOv2 ViT-L from HuggingFace ----
            self.img_model = Dinov2Model.from_pretrained('facebook/dinov2-large')
            # Alternative: torch hub variant with register tokens.
            # self.img_model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitl14_reg')

            # ---------------- end of backbone toggle ----------------

            for param in self.img_model.parameters():
                param.requires_grad = False
            # NOTE: `self.img_model.eval()` is enforced every step by the
            # `train()` override below, so we don't need to call it here.
        ...
        if self.is_pretrained:
            checkpoint = torch.load(v_conf["autoencoder_weights"], weights_only=False, map_location="cpu")["state_dict"]
    ...
    def train(self, mode: bool = True):
        """Keep frozen condition encoders in eval mode even when the parent
        module is switched to train mode. Otherwise stochastic-depth /
        dropout inside DINOv2 would perturb the supposedly fixed condition
    ...
    def apply_denoising_backbone(self, noise_features, img_memory):
    ...
    def diffuse(self, v_feature, v_timesteps, v_condition=None, v_align_feature=None):
        bs = v_feature.size(0)
        de = v_feature.device
        dt = v_feature.dtype
        ...
        # -------------------------
        # alignment branch：改用 clean latent
        # -------------------------
        align_features = self.p_embed(v_align_feature)                # clean CAD latent -> feature
    ...
    def extract_condition(self, v_data):
        ...
        if self.with_img:
            if "img_features" in v_data["conditions"]:
                ...
                # ====================================================
                # Image-condition feature extraction
                # ----------------------------------------------------
                # Must match the backbone toggle in __init__. Both
                # options yield img_feature of shape
                #   [B*num_imgs, 257, 1024]
                # (1 CLS + 16*16 patches, ViT-L hidden dim 1024).
                # ====================================================
                # with torch.no_grad():
                #     # ---- Option A (active): Depth-Anything-V2 backbone ----
                #     backbone_out = self.img_model.forward_features(imgs)
                #     img_feature = torch.cat([
                #         backbone_out["x_norm_clstoken"].unsqueeze(1),
                #         backbone_out["x_norm_patchtokens"],
                #     ], dim=1)

                # ---- Option B (inactive): HuggingFace Dinov2Model ----
                img_feature = self.img_model(imgs).last_hidden_state
                # ---------------- end of feature toggle ----------------
    ...
    def forward(self, v_data, v_test=False, **kwargs):
        ...
        pred, align_loss = self.diffuse(
            ...
            timesteps,
            condition,
            v_align_feature=face_z,   # 新增：对齐分支用 clean latent
        )

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Blender render jobs without bash.")
    parser.add_argument(
        "mode",
        choices=("single-view", "single-view-metal010", "cube24"),
        help="Blender generation mode.",
    )
    parser.add_argument("--num-gpus", type=int, default=RUNTIME.num_gpus)
    parser.add_argument("--max-models", type=int, default=None)
    parser.add_argument("--material-subdir", type=str, default=None)
    parser.add_argument("--output-root", type=Path, default=None)
    parser.add_argument("--samples", type=int, default=RUNTIME.samples)
    parser.add_argument("--mesh-name", type=str, default=PATHS.mesh_name)
    parser.add_argument(
        "--model-list",
        type=Path,
        default=None,
        help="Render this exact model list on the current machine. It is split into per-GPU local rank files.",
    )
    parser.add_argument(
        "--machine-list-dir",
        type=Path,
...
def main() -> None:
    args = parse_args()
    script_name = "render_cube24.py" if args.mode == "cube24" else "render_single_view.py"
    output_root = args.output_root or default_output_root(args.mode)
    material_subdir = args.material_subdir
    if args.mode == "single-view-metal010":
        material_subdir = "Metal010_1K-JPG"
    if args.mode == "cube24" and material_subdir is None:
        material_subdir = "Metal010_1K-JPG"
    ...
    for rank in range(args.num_gpus):
        cmd = [
            str(PATHS.blender_bin),
            "--background",
            "--factory-startup",
            "--addons",
            "cycles",
            "--python",
            str(PATHS.project_root / "blender_scripts" / script_name),
            "--",
            "--model-root",
            ...
            args.mesh_name,
            "--materials-root",
            str(PATHS.materials_root),
            "--output-root",
            str(output_root),
            "--device",
            RUNTIME.blender_device,
            "--samples",
            str(args.samples),
            "--png-compression",
            str(RUNTIME.png_compression),
            "--skip-existing",
        ]
        ...
        log_path = PATHS.logs_root / "blender" / args.mode / f"rank_{rank}.log"
...
def split_for_local_gpus(model_list: Path, num_gpus: int, mode: str) -> list[Path]:
    ...
    output_dir = PATHS.project_root / "runtime_lists" / "local_gpu" / mode / model_list.stem
...
def default_output_root(mode: str) -> Path:
    ...
    if mode == "single-view-metal010":
        return PATHS.blender_single_view_metal010_out

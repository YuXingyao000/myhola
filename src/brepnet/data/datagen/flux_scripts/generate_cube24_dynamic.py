...
# Import shared utilities (works because this is run as -m src.brepnet.data.datagen.flux_scripts.generate_cube24_dynamic)
from src.brepnet.data.datagen.utils import (  # noqa: E402
    resolve_rank as _resolve_rank,
    load_model_ids as _load_model_ids,
    find_model_ids as _find_model_ids,
...
def resolve_rank(explicit_rank: int | None) -> int | None:
    return _resolve_rank(explicit_rank)


def load_model_ids_from_list(list_path: Path) -> list[str]:
    return _load_model_ids(list_path)


def find_model_ids(render_root: Path) -> list[str]:
    return _find_model_ids(render_root)

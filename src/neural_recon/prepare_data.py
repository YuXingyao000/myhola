...
from src.neural_recon.generate_gt import compute_loss, ProgressActor
...
def main(hydra_conf: DictConfig):
    ...
    if True:
        ...
        def read_img(v_path):
            ...
            resized_img = cv2.resize(original_img, trained_img_size, interpolation=cv2.INTER_AREA)
            transform_img = resized_img[:,:,:3]
        ...
        num_sample_per_worker = num_segments // num_worker + 1
        ...
        progress_actor = ProgressActor.remote(num_segments)
    ...
    del gt_loss
    projected_segments = np.concatenate([np.load("output/gt_loss/sub/projected_segments_{}.npy".format(i), allow_pickle=True) for i in range(num_worker)])
    np.save("output/gt_loss/projected_segments", projected_segments)
    del projected_segments
    print("Done")
    pass
